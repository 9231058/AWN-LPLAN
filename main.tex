...
\subtitle{Resource Allocation in CDMA and OFDMA Cellular Networks}
...
\subsection{Basic structure of frames in OFDMA Systems }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{  Dynamic ICIC- A General System Model and Preliminary Concepts}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{System Model and Notations}

	\begin{block}<+->{Uplink Transmission}\justifying{}
		Consider A Multi-cell OFDMA-based Cellular Network consisting of
		\begin{itemize}\justifying{}
			\item In OFDMA, the available bandwidth $W$ is subdivided into $\mid \mathcal{C} \mid$  multiple sub-channels
			\item Thus Bandwidth of each sub-channel $k$ is given by $W^k=\dfrac{W}{\mid \mathcal{C} \mid}$
			\item  A set of $C$ sub-channel	(SC) is denoted by $\mathcal{C}=\{1,2,\ldots,C\}$ (also so called resource block, sub-carrier, channel, or carrier)
		\end{itemize}
	\end{block}
}
